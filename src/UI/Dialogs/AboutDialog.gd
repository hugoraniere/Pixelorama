...
const TRANSLATORS_DICTIONARY := {
	...
	"Felipe Arruda (felipemiguel)": ["Brazilian Portuguese"],
	...
	"陳義昇（ㄚ昇） (love80312)": ["Chinese Traditional"],
...
func _on_about_to_popup() -> void:
	title = tr("About Indie Gen") + " " + Global.current_version
